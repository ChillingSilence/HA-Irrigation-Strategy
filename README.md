...
![Release](https://img.shields.io/badge/Release-2.20.4-green)
...
> **New in 2.20.4 / controller 0.17.3:** every notification and Repairs card ends with an error code such as CS-101, looked up in [Error codes](docs/ERROR_CODES.md) or under Help & tools for what it means, what happens to watering and what to do; notifications name your room and zone the way you did ("Tent · GT4 (Z2)"), and a probe whose reading has simply stopped moving is no longer called dead. **2.20.3 / controller 0.17.2:** notifications call a zone by the name you gave it ("GT4 (Z2)"), a fresh install no longer gets alerts about settings it never made, dropdown lists can be read on a dark theme, and Settings > Repairs says when a setting is not where the controller looks for it. **2.20.2 / controller 0.17.1:** two upgrade fixes from upstream's final review. A room keeps every zone it was set up with even when the controller starts while Home Assistant is still starting, and a controller updated from before 0.16.2 keeps the irrigation times it had saved and always starts, whatever an old state file holds. **2.20.1:** setup shows which version is really running and will not set a room up on code that is waiting for a Home Assistant restart; and a room that is deleted and set up again starts fresh (it no longer inherits the deleted room's settings or its kill switch, and a running controller takes the new room on). **2.19.2** brought fixes from the first real install on a one-zone tent. The controller app and the integration can now be set up in either order (no phantom zones, no false "create the kill switch helper" error), a zone's device carries the name you gave it, "last irrigation" only ever means water, and the menu scrolls on a phone. **New in 2.19:** the dashboard sidebar shows which integration and controller are running, the integration has its icon, and setup asks how each room is plumbed, and a room that says it has a pump is never watered without it. **2.18** brought a setup wizard that keeps what you typed and says exactly what to fix, rooms with a single switch per zone, probes in µS/cm or m³/m³ converted instead of rejected, and setup helpers (unit pickers, substrate presets, catch test, learned-peak suggestion). **2.17** added the recorded-sensor plan graph with the projected P0-P3 day, room on/off, the full P1 ramp, Auto Setpoints, restart-safe setup and a patient pump read-back. Install the matching pair; see the [changelog](CHANGELOG.md).
