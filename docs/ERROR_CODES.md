...
**What it means.** The zone has had its daily water limit (max daily volume), counted from lights-on.

**Watering meanwhile.** Routine shots stop until lights-on starts the next day. Emergency and flush shots are still allowed.
...
- A hold: a setup change waiting (CS-201), plumbing that disagrees (CS-202), a hardware fault (CS-301), or the source-water EC or pH out of range.
...
**What it means.** After a shot, a pump or valve did not confirm it had switched OFF. It may still be running. The controller latches a hold on that hardware, remembered across restarts.
